...
export const enviarCredencialesUsuario = async (usuario, contrasena) => {
  const loginUrl = `${process.env.FRONTEND_URL}/login`;
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Se ha creado o actualizado tu cuenta en la Plataforma Telecuidado.</p>
        
        <h3>Tus Credenciales de Acceso</h3>
        <p><strong>Usuario (RUT sin puntos, ni guión ni dígito verificador):</strong> ${usuario.rut}</p>
        <p><strong>Contraseña Temporal:</strong> ${contrasena}</p>
        ...
        <div style="background-color: #f4f4f4; padding: 10px; border-radius: 5px;">
          ...
          <ul>
            <li>Cambia tu contraseña al primer inicio de sesión</li>
            <li>La contraseña es temporal y debe ser modificada</li>
            <li>Tu rol en el sistema es: ${usuario.rol?.nombre || 'No especificado'}</li>
          </ul>
        </div>
